...
@section('breadcrumb')
    <x-card.breadcrumb title="Home" subtitle="Kategori Artikel" route="{{ route('article.category') }}" />
@endsection
...
@section('content')
    ...
    <div class="card card-body">
                    ...
                    <tr class="search-items">
                        ...
                        <td class="action-btn d-flex gap-2">
                            <a href="javascript:void(0)" class="text-success edit" data-id="{{ $item->id }}"
                                ...
                                onclick="modalEditArticle(this)">
                                <i class="ti ti-pencil fs-5"></i>
                            </a>

                            <x-card.deleted route="{{ route('article.category.destroy', ['id' => $item->id]) }}" />
                        </td>
                    </tr>
                @endforeach
            @endslot
        ...
        <x-modal.basic title="Tambah Kategori" action="{{ route('article.category.store') }}">
            <div class="row">
                <div class="col-12">
        ...
        <x-modal.basic id="EditArticle" title="Edit Kategori" action="{{ route('article.category.store') }}" isUpdate=1>
            <div class="row">
                <div class="col-12">
...
@push('scripts')
    <script>
        function modalEditArticle(element) {
            var id = $(element).data('id');
            var name = $(element).data('name');
            var departement_id = $(element).data('departement_id');

            var route = {!! json_encode(route('article.category.update') . '/') !!} + id

            $("#EditArticle form").attr('action', route)
