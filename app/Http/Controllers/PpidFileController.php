...
class PpidFileController extends Controller
{
    ...
    public function index(string $id)
    {
        try {
            ...
            return view('after-login.ppid.files', compact('files', 'id'));
        } catch (Exception $e) {
            $this->alert(
                'Kategori Berita',
                $e->getMessage(),
    ...
    public function store(Request $request)
    {
        try {
            $request->validate([
                ...
                'file_path' => 'required|mimes:pdf,word,xlxs,png,jpg,jpeg,docx',
            ], [
    ...
    public function update(Request $request, string $id)
    {
        try {
            $request->validate([
                ...
                'release_date' => 'required',
                'ppid_id' => 'required|exists:ppid,id',
                'file_path' => 'sometimes|mimes:pdf,word,xlxs,png,jpg,jpeg,docx',
            ], [
                'file_name.required' => 'Nama File tidak boleh kosong.',
                'release_date.required' => 'Tanggal Rilis tidak boleh kosong.',
                'ppid_id.required' => 'PPID tidak boleh kosong.',
